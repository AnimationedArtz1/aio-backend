...
class VerimorService {
    ...
    static async assignNumberToTenant(tenantId, agentId) {
        ...
        try {
            ...
            const availableNumberResult = await client.query("SELECT id, did_number FROM verimor_numbers WHERE status = 'available' LIMIT 1 FOR UPDATE SKIP LOCKED");
    ...
    static async releaseNumber(didNumber) {
        const client = await pool.connect();
        try {
            console.log('=== VERIMOR RELEASE NUMBER START ===');
            console.log('DID Number:', didNumber);
            await client.query('BEGIN');
            const numberResult = await client.query('SELECT id, tenant_id, agent_id FROM verimor_numbers WHERE did_number = $1', [didNumber]);
            ...
            if (agentId) {
                await client.query('UPDATE agents SET verimor_did = NULL WHERE id = $1', [agentId]);
    ...
    static async getAvailableCount() {
        const result = await pool.query("SELECT COUNT(*) as count FROM verimor_numbers WHERE status = 'available'");
        return parseInt(result.rows[0].count);
    }
    static async addToPool(didNumber, webhookSecret) {
        const result = await pool.query("INSERT INTO verimor_numbers (did_number, status, webhook_secret) VALUES ($1, 'available', $2) RETURNING *", [didNumber, webhookSecret || null]);
        console.log('Numara havuza eklendi:', result.rows[0]);
        return result.rows[0];
    }
}
exports.VerimorService = VerimorService;
