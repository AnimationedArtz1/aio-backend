...
export class AuthService {
  static async login(email: string, passwordRaw: string) {
    ...
    if (!tenant) throw new Error('Kullanıcı bulunamadı.');

    const match = await bcrypt.compare(passwordRaw, tenant.password_hash);
    if (!match) throw new Error('Hatalı şifre.');

    const token = jwt.sign({ id: tenant.id, slug: tenant.slug, email: tenant.email }, JWT_SECRET, { expiresIn: '7d' });
   ...
   */
  static async createTenantFromPayment(paymentData: any) {
    ...
    try {
        ...
        const res = await client.query(
            `INSERT INTO tenants (name, slug, email, password_hash, paynet_ref_code)
              VALUES ($1, $2, $3, $4, $5)
              RETURNING id`,
            [name, slug, email, hash, reference_no]
        );
        const tenantId = res.rows[0].id;

        await client.query(
            `INSERT INTO agents (tenant_id, name, role_type, system_prompt, model)
              VALUES ($1, 'Satış Asistanı', 'general', 'Sen profesyonel bir asistansın.', 'gemini-pro')`,
            [tenantId]
        );

        await client.query(
            `INSERT INTO quotas (tenant_id, plan_name, monthly_message_limit) VALUES ($1, 'pro', 5000)`,
            [tenantId]
        );
    ...
    try {
        ...
        const res = await client.query(
            `INSERT INTO tenants (name, slug, email, password_hash)
              VALUES ($1, $2, $3, $4)
              RETURNING id`,
            [signupData.businessName, slug, signupData.contactEmail, hash]
        );
        const tenantId = res.rows[0].id;
        ...
        const verimorResult = await client.query(
            `SELECT id, did_number
        ...
        const token = jwt.sign(
            { id: tenantId, slug, email: signupData.contactEmail },
            JWT_SECRET,
            { expiresIn: '7d' }
        );
